...
def run_evaluation(cases: list[dict]) -> dict:
    ...
    return {
        ...
        "mean_scalar_accuracy": mean(
            item["metrics"]["scalar_accuracy"] for item in case_results
        ) if case_results else 0.0,
        "mean_macro_list_f1": mean(
            item["metrics"]["macro_list_f1"] for item in case_results
        ) if case_results else 0.0,
        "cases": case_results,
    }
