...
def handle_google_oauth_callback() -> None:
    """Complete hosted Google OAuth only for the beta session that started the flow."""
    ...
    try:
        ...
        if not current_user_id or current_user_id != state_user_id:
            st.query_params.clear()
            st.error("Your JobCopilot login session expired during Google authorization.")
            st.caption(
                "Sign in to JobCopilot again, then restart the Google connection from Settings. "
                "No Google credentials were stored."
            )
            st.stop()
