...
.jc-pill-warn { background: #fff4df; color: #9a5b00; }
.jc-provider {
...
def _authenticated_user() -> dict[str, str] | None:
    ...
    with center:
        ...
        st.caption("Your profile, applications, Google token and agent conversations are isolated from every other tester.")
...
def _reset_agent_chat(user_id: str) -> None:
    ...
    st.session_state["agent_messages"] = []
    st.session_state["agent_telemetry"] = []
    st.success("A fresh private conversation is ready.")
...
def _run_pipeline(user_id: str, job_text: str) -> dict:
    ...
    with capture_llm_telemetry() as events:
        result = jobcopilot_graph.invoke(
            ...
            config={"configurable": {"thread_id": f"private-beta-{user_id}-{uuid4()}"}},
        )
...
def _run_agent_chat_turn(user_id: str, user_input: str) -> None:
    ...
    serialized = serialize_llm_events(events)
    st.session_state["agent_telemetry"] = serialized
...
def _render_provider_trace(events: list[dict], *, compact: bool = False) -> None:
    ...
    provider = summary.get("final_provider") or "unknown"
    model = summary.get("final_model") or "unknown"
    fallback = " · fallback used" if summary.get("fallback_used") else ""
    duration = summary.get("total_duration_ms", 0) / 1000
    st.markdown(
        f"""
        <div class="jc-provider">
          <strong>AI trace:</strong> {escape(str(provider)).title()} · {escape(str(model))}
          · {duration:.2f}s{fallback}<br>
          <span class="jc-muted">{summary.get('successful_calls', 0)} successful call(s), "
          f"{summary.get('failed_attempts', 0)} failed attempt(s). No prompts, outputs or API keys recorded.</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def _render_profile_onboarding(user_id: str) -> bool:
    ...
    with left:
        upload = st.file_uploader("Import my JobCopilot profile", type=["json"], help="The JSON remains in your private workspace.")
        if upload is not None:
            try:
                payload = json.loads(upload.getvalue().decode("utf-8"))
                if not isinstance(payload, list):
                    raise ValueError("The profile file must contain a list of verified facts.")
                required = {"id", "type", "content"}
                if any(not required.issubset(item) for item in payload if isinstance(item, dict)):
                    raise ValueError("Every profile fact must include id, type and content.")
        ...
        if payload:
            ...
            with st.expander("Preview imported facts"):
                for item in payload[:8]:
                    st.write(f"**{item.get('type', 'Fact').title()}** — {item.get('content', '')}")
                if len(payload) > 8:
                    st.caption(f"+ {len(payload) - 8} additional facts")
            if st.button("Activate my profile", type="primary", use_container_width=True):
...
def _render_overview(user: dict[str, str]) -> None:
    ...
    upcoming = [
        ...
        if (reminder := _parse_date(record.reminder_date)) is not None
        and today <= reminder <= today + timedelta(days=7)
    ]
    ...
    with q1:
        st.markdown('<div class="jc-card"><div class="jc-card-title">Analyze a new opportunity</div><p class="jc-muted">Paste an offer and receive a grounded email draft.</p></div>', unsafe_allow_html=True)
        if st.button("Start an application", type="primary", use_container_width=True):
            _request_navigation("New application")
    with q2:
        st.markdown('<div class="jc-card"><div class="jc-card-title">Work with Agent Chat</div><p class="jc-muted">Ask about offers, saved applications or follow-ups.</p></div>', unsafe_allow_html=True)
        if st.button("Open Agent Chat", use_container_width=True):
            _request_navigation("Agent Chat")
    with q3:
        connected = google_token_exists(user_id)
        st.markdown(
            f'<div class="jc-card"><div class="jc-card-title">Google connection</div><p class="jc-muted">Status: <strong>{"Connected" if connected else "Not connected"}</strong></p></div>',
            unsafe_allow_html=True,
        )
    ...
    with right:
        ...
        st.markdown(
            ...
            <div class="jc-card">
              ...
              <p class="jc-muted">Model: {escape(settings.openai_model if settings.llm_provider == 'openai' else settings.anthropic_model)}</p>
...
def _render_analysis(user_id: str) -> None:
    ...
    with st.container(border=True):
        ...
        analyze = st.button("Analyze offer and prepare draft", type="primary", use_container_width=True)

    if analyze:
        if not job_text.strip() or job_text.strip() == DEFAULT_JOB_TEXT:
            st.warning("Paste a complete job description first.")
        else:
            with st.status("Understanding the offer and retrieving verified evidence…", expanded=True) as status:
                try:
                    ...
                    status.update(label="Application workspace ready", state="complete", expanded=False)
                except Exception as exc:
    ...
    overview_tab, match_tab, email_tab = st.tabs(["Role overview", "Match & evidence", "Email & actions"])
    with overview_tab:
        a, b = st.columns(2)
        with a:
            ...
            st.markdown(" ".join(f'<span class="jc-pill">{escape(item)}</span>' for item in analysis.get("required_skills", [])), unsafe_allow_html=True)
        with b:
            st.markdown("#### Tools and stack")
            st.markdown(" ".join(f'<span class="jc-pill">{escape(item)}</span>' for item in analysis.get("tools_and_stack", [])), unsafe_allow_html=True)
            st.markdown("#### Domain focus")
            st.markdown(" ".join(f'<span class="jc-pill">{escape(item)}</span>' for item in analysis.get("domain_focus", [])), unsafe_allow_html=True)
    with match_tab:
        m1, m2 = st.columns(2)
        with m1:
    ...
    with email_tab:
        ...
        subject = st.text_input("Subject", value=draft["subject"], key="beta_email_subject")
        body = st.text_area("Email body", value=draft["body"], height=360, key="beta_email_body")
        st.markdown('<div class="jc-step">Step 3 · Choose the next action</div>', unsafe_allow_html=True)
        recipient = st.text_input("Recruiter email", key="beta_recipient", placeholder="recruiter@company.com")
        ...
        reviewed = st.checkbox("I reviewed the recipient, subject and email body.")
        c1, c2, c3 = st.columns(3)
        with c1:
            if st.button("Save to tracker", use_container_width=True):
                existing = find_existing_application(analysis["company"], analysis["role"], user_id=user_id)
                if existing:
                    st.warning("This application already exists in your tracker.")
                else:
                    record = create_application_record(
                        company=analysis["company"], role=analysis["role"],
                        email_subject=subject, email_body=body, source="private-beta",
        ...
        with c2:
            if st.button("Create Gmail draft", type="primary", use_container_width=True):
                if not reviewed or not recipient.strip():
                    st.warning("Review the message and enter the recruiter email first.")
                else:
                    try:
                        result = create_gmail_draft(to=recipient, subject=subject, body=body, user_id=user_id)
            ...
            if st.button("Add follow-up", use_container_width=True):
                try:
                    payload = build_followup_event_payload(
                        company=analysis["company"], role=analysis["role"], followup_date=str(followup_date)
                    )
...
def _render_agent_chat(user_id: str) -> None:
    ...
    st.caption("A tenant-isolated copilot for applications, drafts and follow-ups.")
    ...
    if not messages:
        st.info("Paste a job offer or ask about your saved applications. External Gmail and Calendar actions still require explicit confirmation.")
    ...
    _render_provider_trace(st.session_state.get("agent_telemetry", []), compact=True)
...
def _render_applications(user_id: str) -> None:
    ...
    with filter_col:
        status_filter = st.selectbox("Status", ["All", *STATUS_LABELS.values()])
    with search_col:
        query = st.text_input("Search", placeholder="Company or role")

    filtered = []
    for record in records:
        if status_filter != "All" and STATUS_LABELS[record.status] != status_filter:
            continue
        if query.strip() and query.lower() not in f"{record.company} {record.role}".lower():
            continue
    ...
    for index, record in enumerate(reversed(filtered)):
        with st.expander(f"{record.company} — {record.role} · {STATUS_LABELS[record.status]}"):
            c1, c2 = st.columns(2)
            with c1:
            ...
            with c2:
                current_reminder = _parse_date(record.reminder_date) or date.today()
                new_reminder = st.date_input(
                    "Reminder", value=current_reminder,
                    key=f"reminder-{index}-{record.company}-{record.role}",
                )
            notes = st.text_area("Notes", value=record.notes, key=f"notes-{index}-{record.company}-{record.role}")
            if st.button("Update application", key=f"update-{index}-{record.company}-{record.role}"):
                updated = update_application_record(
                    record.company, record.role, user_id=user_id,
                    status=new_status, reminder_date=str(new_reminder), notes=notes,
                )
                if updated:
...
def _render_settings(user_id: str) -> None:
    ...
    with c1:
        with st.container(border=True):
            ...
            st.caption("Each tester receives a separate OAuth token. No other account can use it.")
            if st.button("Connect or refresh Google", type="primary", use_container_width=True):
                try:
    ...
    with c2:
        with st.container(border=True):
            ...
            st.write(f"Fallback: **{settings.llm_fallback_provider.title() if settings.llm_fallback_provider else 'Disabled'}**")
            st.caption("Telemetry stores provider, model, operation, status, latency and available token counts. It never stores prompts, generated content, API keys or raw provider errors.")
    ...
    with st.expander("Danger zone"):
        st.warning("Deleting your workspace removes profile memories, applications, FAISS index and Google token.")
        confirmation = st.text_input("Type DELETE to confirm", key="delete_confirmation")
        if st.button("Delete my private workspace", use_container_width=True):
...
def _render_sidebar(user: dict[str, str]) -> str:
    ...
    with st.sidebar:
        st.markdown('<div class="jc-brand">JobCopilot<span class="jc-brand-dot">.</span></div>', unsafe_allow_html=True)
        st.caption("Private beta workspace")
        st.divider()
        page = st.radio("Navigation", NAV_ITEMS, key="nav_radio", label_visibility="collapsed")
        st.divider()
